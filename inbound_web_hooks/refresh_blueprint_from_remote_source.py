...
3. Configure the webhook to use either token based authentication or Default CB
   API authentication.
4. Send a REST request using the selected authentication. The following
...
def inbound_web_hook_post(*args, parameters={}, **kwargs):
    ...
    if remote_source_password:
        set_progress(f"Updating Blueprint with Remote Source Password")
