...
def run(job, **kwargs):
    ...
    if resource:
        ...
        # Get Tags from Resource Handler. If matching attrs exist on the
        # resource then include them as tags for each resource
        ...
        if len(tags_to_apply) > 0:
            i = 0
            while i < len(template["resources"]):
                template["resources"][i]["tags"] = tags_to_apply
                i += 1
